...
class FrontendController extends Controller
{
    ...
    {
        $sliders = PortFolio::where('type', 'slider')
            ->where('status', 1)
        ...
        $banners = PortFolio::where('type', 'slider')
            ->where('status', 1)
            ->where(function($query) {
...
// Eğer önceki URL login rotası ise
    ...
    public function paytrOdemeBildirim(Request $request)
    {
        ...
        //###### Bu kısımda herhangi bir değişiklik yapmaya gerek yoktur. #######
        //
        ...
        if ($post['status'] == 'success') { //# Ödeme Onaylandı
            Order::where('merchant_oid', $merchant_oid)->update(['status' => 'completed']);

        } else { //# Ödemeye Onay Verilmedi

            Order::where('merchant_oid', $merchant_oid)
    ...
    {
        $ilce = [];
        $ilceler_data = City::where('id',$request->id)->pluck('content')->first();
